...
#
# TYPO3 Extension Test Runner - nr_landingpage
...
loadHelp() {
    read -r -d '' HELP <<EOF
nr_landingpage - TYPO3 Extension Test Runner
...
case ${TEST_SUITE} in
    architecture)
        ...
        COMMAND="php ${PHP_OPCACHE_OPTS} -dxdebug.mode=off .Build/bin/phpstan analyse -c phpstan.neon"
...
import sys, json; print(json.load(sys.stdin)['raw'].get('primary_url', ''))" 2>/dev/null)
    ...
    lint)
        ...
        # linted along with everything else and the suite died on
        # class-alias-loader's alias-loader-include.tmpl.php, which is a template
        # and deliberately not valid PHP. Pruning also skips walking the vendor
        # tree instead of walking it and discarding the results.
    ...
    rector)
        if [ "${CGLCHECK_DRY_RUN}" -eq 1 ]; then
            COMMAND="php ${PHP_OPCACHE_OPTS} -dxdebug.mode=off .Build/bin/rector process --config rector.php --dry-run"
        else
            COMMAND="php ${PHP_OPCACHE_OPTS} -dxdebug.mode=off .Build/bin/rector process --config rector.php"
        fi
